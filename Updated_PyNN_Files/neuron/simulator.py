    ...
    def _build_cell(self, cell_model, cell_parameters):
        ...
        if isinstance(self._cell.source, dict):
            for k, v in self._cell.source.items():
                ...
                elif k=='end_axon_node':
                    state.register_gid(gid+3e6, self._cell.source['end_axon_node'], section=self._cell.source_section['end_axon_node'])
                elif k=='collateral':
...
class Connection(common.Connection):
    ...
    def __init__(self, projection, pre, post, **parameters):
        ...
        elif projection.source=='end_axon_node':
            self.presynaptic_cell = projection.pre[pre] + 3e6
        else:
